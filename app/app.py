...
def load_or_generate_data():
    ...
    try:
        ...
        if DATA_PATH.exists():
            st.success("✅ Dataset found. Loading data...")
            return pd.read_csv(DATA_PATH)

        # Step 2: If missing → generate
        st.warning("⚠️ Dataset not found. Generating dataset...")

        # Create data folder if not exists
        DATA_PATH.parent.mkdir(parents=True, exist_ok=True)

        # Import and generate
        from src.data_generation import generate_ward_data
        from data.synthetic_data import save_city_ward_dataset

        df = generate_ward_data()
        save_city_ward_dataset(df, DATA_PATH)

        # Step 3: Load and confirm
        if DATA_PATH.exists():
